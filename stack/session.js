...
/**
 * Provide session in request object
 */
...
module.exports.session = function(options) {
  ...
  return require('cookie-sessions')(options);
...
/**
 * Provide user capabilities (context) in request object
 */

module.exports.context = function(options) {
  ...
  return function handler(req, res, next) {
    ...
    // use static context
      ...
      req.context = options.guest || {};
      // user authenticated?
      if (req.session && req.session.uid) {
        // provide user context
        req.context = options.user || req.context;
      }
